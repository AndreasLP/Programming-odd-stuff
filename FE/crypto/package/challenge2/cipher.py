...
class KeyStream:
    ...
    def __init__(self, psw, crypt='enc'):
        if crypt == 'enc':
...
def decrypt(msg, psw, crypt='enc'):
    plain = encrypt(msg, psw, crypt=crypt)
    return plain


def encrypt(msg, psw, crypt='enc'):
    keystream = KeyStream(psw, crypt=crypt)
    cipher = []
    for char in msg:
...
def main():
    ...
    if mode == "enc":
        ...
        print "Decrypting (type 2) file %s to %s" % (ifile, ofile)
        cnt = cnt[3:]
        cnt = decrypt(cnt, psw, crypt='dec2')
        # If decrypted text do not start with MAGIC bytes
        # decryption has failed
        if cnt[:len(MAGIC)] != MAGIC:
            print "Decryption failed! Probably wrong password specified."
